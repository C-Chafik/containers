...
/*   Updated: 2022/08/17 13:34:18 by cmarouf          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
namespace	ft
{
    ...
    class RBT
    {
        public:
			...
			typedef typename ft::SET_RBT_iterator<Node>									iterator;
			typedef typename ft::const_SET_RBT_iterator<Node>							const_iterator;
			...
			ft::pair<iterator,bool> erase( const value_type & key )
			{
				Node_pointer del = _root;

				while ( del && del->data != key )
				{
					if (_cmp(key, del->data))
						del = del->left;
					else if (_cmp(del->data, key))
						del = del->right;
					else
						break ;
				}
